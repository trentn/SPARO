...
group1 = lookup.get_group_from_names(['Robot A'], ['J2'])
...
command1 = hebi.GroupCommand(group1.size)
...
positions1 = np.empty(group1.size, dtype=np.float64)
while t < 10.0:
  for module_index in range(0, group.size):
    positions[module_index] = sin((t) + (module_index))
    positions1[module_index] = sin((t) + (module_index))
  command.position = positions
  command1.position =  positions1
  group.send_command(command)
  sleep(1)
  group1.send_command(command1)
  sleep(period)
  t = t + period
...
while t < 10.0:
  ...
  command1.position = -positions
  if group1.send_command_with_acknowledgement(command):
    print('Got acknowledgement.')
  else:
    print('Did not receive acknowledgement!')
