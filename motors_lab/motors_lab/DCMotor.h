...
enum DCMOTOR_STATE{NONE, ANGLE, SPEED};

class DCMotor {
  public:

    int dcmotor_control = NONE;
    double speed = 0;
    double angle = 0;
    ...
    update(){
      switch(dcmotor_control) {
        case ANGLE: setAngle(angle);
                    break;
        case SPEED: speed_Control(speed);
                    break;
      }
    }
    ...
    void guiCommand(String command) {
      switch(command.charAt(0)) {
        case 'A': angle = command.substring(2).toDouble();
                  dcmotor_control = ANGLE;
                  break;
        case 'S': speed = command.substring(2).toDouble();
                  dcmotor_control = SPEED;
                  break;
      }
    }
